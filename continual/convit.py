...
class ClassAttention(nn.Module):
    ...
    def forward(self, x, mask_heads=None, **kwargs):
        ...
        # x_cls = x_cls[:, 0:1] # Only cls token for classification

        # x_cls = x_cls[:, 0:1] # 放到后面试试
...
class Block2(nn.Module):
    ...
    def __init__(self, dim, num_heads,  mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_type=GPSA,
                 fc=nn.Linear, deep=False, task_index=1, **kwargs):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = attention_type(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, fc=fc, deep=deep,**kwargs)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, fc=fc)
    ...
    def forward(self, x, mask_heads=None, task_index=1, attn_mask=None):
        if isinstance(self.attn, ClassAttention2) or isinstance(self.attn, JointCA):  # Like in CaiT
...
class ClassAttention2(nn.Module):
    # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
    # with slight modifications to do CA
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., fc=nn.Linear, deep=False):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5
    ...
    def forward(self, x, mask_heads=None, **kwargs):
        ...
        if self.deep:
          q = self.q(x[:,:4]).unsqueeze(1).reshape(B, 4, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        else:
          q = self.q(x[:, 0]).unsqueeze(1).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        if mask_heads is not None:
            ...
            attn = attn * mask_heads
        if self.deep:
           x_cls = (attn @ v).transpose(1, 2).reshape(B, 4, C)
        else:
           x_cls = (attn @ v).transpose(1, 2).reshape(B, 1, C)
        x_cls = self.proj(x_cls)
        x_cls = self.proj_drop(x_cls)
