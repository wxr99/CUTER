...
from .nus_loader import NUSWIDELoader
...
def build_dataset(dataset_name, root_dir, low_range, high_range, phase=None, year='2014', transform=None):
    ...
    if 'coco' in dataset_name:
        ...
        if phase == 'train':
            dataset = VOC('07', 'edgeboxes', 'trainval', included=retrieve_classes, root='./datasets/VOC2007_train/',
                          transform=transform)
        if phase == 'val':
            dataset = VOC('07', 'edgeboxes', 'test', included=retrieve_classes, root='./datasets/VOC2007_test/',
                          transform=transform)

    elif dataset_name == 'nuswide':
        if phase == 'train':
            dataset = NUSWIDELoader(root='./datasets/nuswide', img_root='./datasets/nuswide/Flickr', split='train', included=retrieve_classes,
                                    transform=transform)
        if phase == 'val':
            dataset = NUSWIDELoader(root='./datasets/nuswide', img_root='./datasets/nuswide/Flickr', split='val', included=retrieve_classes,
                                    transform=transform)
    return dataset
