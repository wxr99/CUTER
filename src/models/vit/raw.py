...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        output = self.head(x)
        for i in range(5):
            name = '{}nsce_sparse.csv'.format(i)
            # sparsity = (torch.norm(self.head.weight[2*i:2*i+1], p=1) / torch.norm(self.head.weight[8:9], float('inf'))/2/784).detach().cpu().numpy()
            sparsity = (torch.norm(self.head.weight[2 * i:2 * i + 1], float('inf')) / (
                        torch.norm(self.head.weight[2 * i:2 * i + 1], p=1) / 2 / 784)).detach().cpu().numpy()
            with open(name, 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([sparsity])
