...
# Reservoir Sampling
class PRS_Buffer():
    def __init__(self, total_classes, buffer_size) -> None:
        self.buffer = torch.tensor([]).cuda()
        self.label = torch.tensor([]).cuda()
        self.budget_size = buffer_size
        self.buffer_freq = torch.ones(total_classes).cuda()

    def add(self, x, y, low_range, high_range, allocate_prob, class_freq):
        print(self.buffer_freq)
        if len(self.buffer) == 0:
            ...
            self.buffer_freq[low_range:high_range] += y.sum(dim=0)[low_range:high_range]
            ...
            m = self.budget_size * allocate_prob
            n = class_freq
            mem_freq = torch.sum(self.label, dim=0)
            delta = mem_freq - allocate_prob * sum(mem_freq)
            delta = torch.softmax(delta, dim=0)

            for i, l in enumerate(y):

                p = random.random()
                w = l * torch.exp(-(0.01*n)) / (torch.sum(l * torch.exp(-0.01*n)))
                select_prob = ((m+0.0001) * w / (n+0.0001)).sum()
                if p <= select_prob:
                    index = random.randint(0, len(self.buffer) - 1)
                    mask = torch.ones_like(l)
                    mask[low_range:high_range] = 0
                    self.buffer_freq[self.label[index]>0] -= self.label[index][self.label[index]>0]
                    self.label[index] = l - mask.cuda()
                    self.buffer[index] = x[i]
                    self.buffer_freq[low_range:high_range] += l[low_range:high_range]

                    # for index, _ in enumerate(self.label):
                    #     k = random.random()
                    #     out_prob = sum(delta * self.label[index])
                    #     if k <= out_prob:
                    #         mask = torch.ones_like(l)
                    #         mask[low_range:high_range] = 0
                    #         self.label[index] = l - mask.cuda()
...
class MultiLabelIncremental_PRS:
    def __init__(self, args):
        print('Running MLIL PRS')
        ...
        if self.replay:
            self.mem_buffer = PRS_Buffer(self.total_classes, args.buffer_size)
        self.class_freq = torch.zeros(self.total_classes).cuda()
        self.allocation_prob = torch.zeros(self.total_classes).cuda()
    ...
    def _train_one_epoch(self, train_loader, scaler, low_range, high_range, epoch):
        ...
        for i, (image, target) in enumerate(train_loader):
            ...
            freq = torch.sum(target, dim=0)
            mask = torch.zeros_like(freq)
            mask[low_range:high_range] = 1
            self.class_freq += freq * mask
            self.allocation_prob = self.class_freq ** self.rho / (self.class_freq ** self.rho).sum()
            self.total_num += len(target)
            self.mem_buffer.add(image, target, low_range, high_range, self.allocation_prob, self.class_freq)
            # ----------
            # forward
            # ----------
            with autocast():  # mixed precision
                output = self.model(image)['logits']  # sigmoid will be done in loss !

            loss, cls_loss = self.compute_loss(output, target, low_range, high_range)

            if len(self.mem_buffer.buffer) > self.mem_batch_size:
                memory = TensorDataset(self.mem_buffer.buffer, self.mem_buffer.label)
                ...
                mem_output = self.model(mem_img)['logits']
    ...
    def train(self):
        ...
        for low_range, high_range in incremental_stages:
            ...
            #         train_dataset_with_old = torch.utils.data.ConcatDataset(
            #             [train_dataset, self.old_dataset.get_bufferset(self.args.buffer_size)])
            #     else:
            ...
            mean_p_c, mean_r_c, mean_f_c, precision_o, recall_o, f1_o = metrics[0], metrics[1], metrics[2], metrics[3], metrics[4], metrics[5]
        ...
        if self.rank == 0:
            ...
            print_to_excel(self.excel_path, expe_name, ds_name, self.base_classes,
                           self.task_size, self.total_classes, params, mAP_list, cf1_list, of1_list, metrics,
                           git_hash=None)
