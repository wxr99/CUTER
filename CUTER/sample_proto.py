...
def _forward_features(model, loader):
    ...
    for i, (image, target) in enumerate(loader):
        ...
        with torch.no_grad():
            feature = model(image)['pool_embeddings']
...
def icarl_sample_protos_buffer(model, low_range, high_range, dataset, loader, num_protos, image_id_set=None, logger=None):
    """
    ...
    if num_protos < 0:
        raise ValueError("num_protos should be positive!! receive num_protos {}".format(num_protos))
    elif num_protos < 1:
        # total_num_protos = num_protos * len(dataset)
        # num_protos =  int(total_num_protos / (high_range - low_range))
        pass
    else:
    ...
    herding_ds = torch.utils.data.Subset(dataset, all_herding_index)

    return herding_ds, per_cls_index
