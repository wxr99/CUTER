...
class MultiLabelIncremental_Base:
    def __init__(self, args):
        ...
        self.logger.info('Running MLIL base')
        ...
        self.kd_loss = False
        self.total_class_freq = torch.zeros(self.total_classes).cuda()
        # pseudo label
    ...
    def compute_loss(self, output, target, low_range, high_range):
        """
    ...
    def _train_one_epoch(self, train_loader, scaler, low_range, high_range, epoch):
        ...
        for i, (image, target) in enumerate(train_loader):
            ...
            self.total_class_freq += torch.sum(target, dim=0)
            # print(self.total_class_freq)
            # ----------
            # forward
            # ----------
    ...
    def _train_task(self, initial_epoch, nb_epochs, low_range, high_range, train_loader,
        ...
        self.optimizer = torch.optim.AdamW(params=parameters, lr=self.lr, weight_decay=0)  # true wd, filter_bias_and_bn
