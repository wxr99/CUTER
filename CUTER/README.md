...
## Train on COCO
    ...
    --options configs/multi_tresnetm_pod.yaml --output_name MICIL_COCO_POD

##  Train on VOC

    # single node multi processes  
    python -m torch.distributed.launch --nproc_per_node 2 --master_port=21000 main.py \
    --options configs/multi_tresnetm_pod_voc.yaml --output_name MICIL_VOC_POD 
...
## Train on COCO
    ...
    # single node multi processes  
    python -m torch.distributed.launch --nproc_per_node 2 --master_port=21000 main.py \
    --options configs/multi_tresnetm_krt.yaml --output_name MICIL_COCO_KRT
     
##  Train on VOC

    # single node multi processes  
    python -m torch.distributed.launch --nproc_per_node 2 --master_port=21000 main.py \
    --options configs/multi_tresnetm_krt_voc.yaml --output_name MICIL_VOC_KRT
