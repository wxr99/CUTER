...
# from detectron2.detectron2.data import print_instances_class_histogram
...
# Reservoir Sampling
class OCDM_Buffer():
    def __init__(self, buffer_size, total_classes, rho) -> None:
        self.buffer = torch.tensor([]).cuda()
        self.label = torch.tensor([]).cuda()
        self.budget_size = buffer_size
        self.class_freq = torch.ones(total_classes).cuda()
        self.class_prob = torch.zeros(total_classes).cuda()
        self.rho = rho
    ...
    def add(self, x, y, total_num, low_range, high_range):
        ...
        hot_labels = y * (1-mask)
        y = y -  mask.cuda()
        # if torch.sum(self.class_freq, dim=0) > 0:
        #     self.class_prob = self.class_freq ** self.rho / torch.sum(self.class_freq ** self.rho, dim=0)
        if len(self.buffer) == 0:
            ...
            for i in np.arange(len(y)):
                divergence_list = []
                for j in np.arange(len(self.buffer)):
                    y = self.label[j]
                    y[y < 0] = 0
                    temp_freq = self.class_freq - y
                    temp_prob = temp_freq / torch.sum(temp_freq, dim=0)
                    target_prob = self.class_freq ** self.rho / torch.sum(self.class_freq ** self.rho, dim=0)
                    divergence = F.kl_div(target_prob.log(), temp_prob, reduction='sum')
                    divergence_list.append(divergence.cpu())
                index = np.argmax(divergence_list)
                self.buffer = torch.cat([self.buffer[:index], self.buffer[index+1:]], dim=0)
                self.label = torch.cat([self.label[:index], self.label[index+1:]], dim=0)



class MultiLabelIncremental_OCDM:
    def __init__(self, args):
        ...
        self.logger.info('Running MLIL OCDM')
        ...
        if self.replay:
            self.mem_buffer = OCDM_Buffer(args.buffer_size, args.total_classes, args.rho)
    ...
    def _train_one_epoch(self, train_loader, scaler, low_range, high_range, epoch):
        ...
        for i, (image, target) in enumerate(train_loader):
            ...
            self.mem_buffer.add(image, target, self.total_num, low_range, high_range)
            # ----------
            # forward
            # ----------
